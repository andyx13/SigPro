...
class Pipeline(ABC):
    ...
    def _apply_pipeline(self, window, is_series=False):
        ...
        #print(context.keys(), amplitude_values[:5], 'ctx')
    ...
    #Idea in process_signal: if multiple primitives require a single context arg, try adding those as extra columns?
...
class LayerPipeline(Pipeline):
    ...
    def _build_pipeline(self):
        ...
        for layer in range(1, 1 + self.num_layers):
            for combination in self.primitive_combinations:
                ...
                if layer > combination_length:
                    continue
                if (tuple(combination[:layer]) not in prefixes) or layer == combination_length: #If we haven't processed this prefix yet, or if it's a terminal output
                    ...
                    final_primitives_list.append(final_primitive.get_name()) #build the primitives list
                    ...
                    if layer == 1:
                        #print('\n\n', combination, numbered_primitive_name, '\n\n ')
                        final_primitive_inputs[numbered_primitive_name]['amplitude_values'] = 'amplitude_values'
                    else:
                        #Get input name
                        input_column_name = '.'.join([prim.get_tag() for prim in combination[:layer - 1]]) + f'.{layer-1}' #.amplitude_values'
                    ...
                    if layer <= combination_length - 1: #self.num_layers - 1:
                        output_column_name +=   '.' + str(layer) #don't include the layer number in the final output.
                        for output_dict in final_primitive.get_outputs(): #final primitive(aggregation): output the aggregation name.
        ...
        # print(final_primitives_list)
        # print()
        # print(final_init_params)
        # print()
        # print(final_primitive_inputs)
        # print()
        # print(final_primitive_outputs)
        # print() #'/n/newfewf')
        # print(final_outputs)
        # print()
        return MLPipeline( 
